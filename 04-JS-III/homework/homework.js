...
function diaDeLaSemana(numeroDeDia) {
    ...
    //Escribe tu código aquí
    if (numeroDeDia === 2 || numeroDeDia === 7) return "Es fin de semana";
...
function empiezaConNueve(n) {
    ...
    //Escribe tu código aquí
    let digit = Math.floor(n / Math.pow(10, n.toString().length - 1));
    return digit === 9;
}
...
function mesesDelAño(array) {
    ...
    let new_array = [];
    for (let i = 0; i < array.length; i++) {
        if (array[i] === "Enero" || array[i] === "Marzo" || array[i] === "Noviembre") new_array.push(array[i]);
    }
...
function mayorACien(array) {
    ...
    // Tu código:
    let new_array = [];
    for (let i = 0; i < array.length; i++) if (array[i] > 100) new_array.push(array[i]);
    return new_array;
